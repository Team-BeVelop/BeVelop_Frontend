...
const  Login =  () => {
    ...
    const signInBtn =() =>{
        ...
        if (Users.action.type === 'AUTH_ERROR')alert('로그인에 실패하였습니다');
        OnclickPopUp();
    };
    ...
    return(
        <Wrap>
            ...
            <Button onClick={OnclickPopUp}><IoIosClose/></Button>
                ...
                {signUp ? <InputBox>
                    <p>이메일</p>
                    <input onChange={handleSignUpInput} value={signUpInput.id} name="id" type="text" placeholder="이메일을 입력해주세요"></input>
                    <p>닉네임</p>
                    <input onChange={handleSignUpInput} value={signUpInput.nickName} name="nickName" type="text" placeholder="닉네임을 입력해주세요"></input>
                    <p>비밀번호</p>
                    <input onChange={handleSignUpInput} value={signUpInput.pw} name="pw" type="password" placeholder="비밀번호를 입력해주세요"></input>
                    
                    <Box color='#888A92' bgColor = "#F2F4F6" onClick={signUpBtn}>회원가입</Box>
                </InputBox> : <LoginKind>
                    <Box color='#3C1E1E' bgColor = "#F7E317">카카오로 시작하기</Box>
                    <Box color='#FFFFFF' bgColor = "#24292F">Github로 시작하기</Box>
                    <div className="or" >또는</div>
                    <Box color='#FFFFFF' bgColor = "#7A5DF5" onClick={()=>setSignUp(true)}>이메일로 가입하기</Box>
                    <p>이미 계정이 있으신가요?<a>로그인</a></p>
                </LoginKind>
                }
